...
public class DrawView extends View implements OnTouchListener, Serializable
{
	...
	public DrawView(Context context, WindowManager wm, int diceSoundKey, int hitMsgKey) {
		super(context);
		setFocusable(true);
	...
	{		
		...
		String hit_text = this.getContext().getString(/*R.string.*/hitMsgKey);	
		...
		{			
			if(mp == null){
				mp = MediaPlayer.create(this.getContext(), /*R.raw.*/diceSoundKey);
			}
